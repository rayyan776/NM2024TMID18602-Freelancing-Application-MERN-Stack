...
const ProjectWorking = () => {
  ...
  const fetchChats = async () => {
    await axios.get(`http://localhost:6001/fetch-chats/${params['id']}`).then((response) => {
  ...
  useEffect(() => {
    ...
    return () => {
      socket.off('message-from-user', fetchChats); // Cleanup listener on unmount
    };
  }, [socket]);
                ...
                {chats ? (
                  <div className="chat-messages">
                    {chats.messages?.map((message) => (
                      <div
                        className={
                          message.senderId === localStorage.getItem('userId') ? 'my-message' : 'received-message'
                        }
                        key={message.id}
                      >
                        <div>
